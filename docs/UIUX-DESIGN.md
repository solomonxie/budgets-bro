...
- A form's optional part lives behind one row named for what's in it (the spend form's **Items**) that looks and behaves like any other picker row (label, summary of what is inside, chevron that turns). The collapsed row's value is the data itself (the item names), never a count. Inside, typing comes first: plain name and price inputs, always one blank row for the next item, past names as chips under the one being typed — no picker to open, no "add" button. What most entries fill in stays an always-visible row: the memo, one line, Return closes it. Hide the section entirely when the current mode leaves nothing inside it.
