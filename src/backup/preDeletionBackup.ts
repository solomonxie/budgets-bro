...
import type { SyncOutcome } from '../sync/cloudSync';
...
// not block the wipe, but its outcome is returned so the caller can say so.
export async function backUpBeforeDeletion(db: SQLiteDatabase, boards: Board[], at = new Date()): Promise<SyncOutcome[]> {
  ...
  return uploadNamedBackups(db, files);
}
