...
export async function importAppExport(db: SQLiteDatabase, files: PickedAppExport): Promise<AppExportImportResult> {
  ...
  await db.withTransactionAsync(async () => {
    ...
    for (const t of files.transactions) {
      ...
      await db.runAsync(
        `INSERT INTO transactions (board_id, account_id, category_id, payee_id, memo, amount_cents, date, transfer_account_id, import_id, purchase_items, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        boardId,
        newAccountId,
        newCategoryId,
