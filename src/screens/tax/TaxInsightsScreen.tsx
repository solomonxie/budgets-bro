...
export function TaxInsightsScreen() {
  ...
  const [interestTotals, setInterestTotals] = useState({ incomeCents: 0, expenseCents: 0 });
  const [donationCategoryIds, setDonationCategoryIds] = useState<number[]>([]);
  const [donationCents, setDonationCents] = useState(0);
  const [trackingGains, setTrackingGains] = useState<{ accountId: number; name: string; gainCents: number; sinceDate: string | null }[]>([]);
  ...
  useEffect(() => {
    (async () => {
      const db = await getDb();
      setInterestTotals(await reportsRepo.categoryIncomeAndExpenseInRange(db, boardId, interestCategoryIds, `${year}-01-01`, `${Number(year) + 1}-01-01`));
    })();
  ...
  useEffect(() => {
    (async () => {
      const db = await getDb();
      setDonationCents(await reportsRepo.categorySpendingInRange(db, boardId, donationCategoryIds, `${year}-01-01`, `${Number(year) + 1}-01-01`));
    })();
  ...
  useEffect(() => {
    (async () => {
      const db = await getDb();
      const trackingAccounts = accounts.filter((a) => accountKind(a.account.type) === 'Tracking');
      const yearStart = `${year}-01-01`;
      const results = await Promise.all(
        trackingAccounts.map(async (a) => {
          const history = [...(await accountValueHistoryRepo.listValueHistory(db, a.account.id))].sort((x, y) =>
            x.effectiveDate.localeCompare(y.effectiveDate),
          );
          if (history.length === 0) return { accountId: a.account.id, name: a.account.name, gainCents: 0, sinceDate: null };
          ...
          const priorEntry = [...history].reverse().find((h) => h.effectiveDate < yearStart);
          const baseline = priorEntry ?? history[0];
          const current = history[history.length - 1];
          const sinceDate = priorEntry ? null : baseline.effectiveDate;
          return { accountId: a.account.id, name: a.account.name, gainCents: current.valueCents - baseline.valueCents, sinceDate };
        }),
      );
  ...
  const provinceName = province ? CANADA_PROVINCES.find((p) => p.code === province)?.name ?? province : null;
  ...
  const toggleInterestCategory = (id: number) => {
    ...
    (async () => {
      const db = await getDb();
      await settingsRepo.setJsonSetting(db, interestCategoriesKey(boardId), next);
    })();
  };
    ...
    <ScreenContainer scroll>
      <View style={styles.card}>
        ...
        <View style={styles.row}>
          ...
          <Text style={styles.linkText} onPress={() => setCountryPickerOpen(true)}>
            {TAX_COUNTRIES.find((c) => c.code === country)?.name ?? country} ▾
          </Text>
        </View>
        <View style={styles.row}>
          <Text style={styles.rowLabel}>{t('taxInsights.provinceLabel')}</Text>
          <Text style={styles.linkText} onPress={() => setProvincePickerOpen(true)}>
      ...
      <View style={styles.card}>
        <Text style={styles.label}>{t('taxInsights.investmentGainsLabel', { year })}</Text>
        {trackingGains.length === 0 ? (
          <Text style={styles.hint}>{t('taxInsights.noTrackingAccounts')}</Text>
        ) : (
          trackingGains.map((g) => (
            <Row
              key={g.accountId}
              label={g.sinceDate ? `${g.name} (${t('taxInsights.gainSince', { date: g.sinceDate })})` : g.name}
              value={formatMoney(g.gainCents)}
            />
          ))
        )}
        {trackingGains.length > 0 ? <Row label={t('taxInsights.totalGains')} value={formatMoney(totalGainCents)} /> : null}
        <Text style={styles.hint}>{t('taxInsights.investmentGainsHint')}</Text>
      </View>
      ...
      <View style={styles.card}>
        ...
        <Text style={styles.hint}>{t('taxInsights.estimatedTaxableIncomeHint')}</Text>
      </View>

      {country === 'CA' ? (
        <View style={styles.card}>
          <Text style={styles.label}>{provinceName ? t('taxInsights.timelineLabel', { province: provinceName }) : t('taxInsights.timelineNoProvince')}</Text>
          <Text style={styles.hint}>{t('taxInsights.slipsReadyBy', { date: slipsDate })}</Text>
          <Text style={styles.hint}>{t('taxInsights.rrspDeadline', { year, date: rrspDate })}</Text>
          ...
          <FormRow show={province === 'QC'} label={t('taxInsights.formTP1')} />
        </View>
      ) : null}

      <Pressable style={styles.aiButton} onPress={() => Alert.alert(t('taxInsights.aiSummaryTitle'), t('taxInsights.aiSummaryMessage'))}>
      ...
      <Modal visible={countryPickerOpen} transparent animationType="slide" onRequestClose={() => setCountryPickerOpen(false)}>
        <BottomSheet title={t('taxInsights.countryLabel')} onClose={() => setCountryPickerOpen(false)}>
          {TAX_COUNTRIES.map((c) => (
            <DropdownOption key={c.code} label={c.name} selected={country === c.code} onPress={() => saveCountry(c.code)} />
          ))}
          <Text style={styles.hint}>{t('taxInsights.countryComingSoon')}</Text>
        </BottomSheet>
      </Modal>

      <Modal visible={provincePickerOpen} transparent animationType="slide" onRequestClose={() => setProvincePickerOpen(false)}>
        <BottomSheet title={t('taxInsights.provinceLabel')} onClose={() => setProvincePickerOpen(false)}>
          {CANADA_PROVINCES.map((p) => (
            <DropdownOption key={p.code} label={p.name} selected={province === p.code} onPress={() => saveProvince(p.code)} />
          ))}
        </BottomSheet>
      </Modal>
...
function FormRow({ show, label }: { show: boolean; label: string }) {
  return <Text style={[styles.hint, show ? styles.formRowActive : styles.formRowInactive]}>{show ? '☑' : '☐'} {label}</Text>;
}
...
const styles = StyleSheet.create({
  ...
  rowLabel: { fontSize: 14, color: colors.text, flex: 1, marginRight: spacing.sm },
