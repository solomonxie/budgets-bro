...
- [x] Exchange Rates page (Insights) — a board of currencies rather than a From and a To: tap one to type into it on the app's own calculator pad and every other row restates the same money, with the list reorderable, removable and remembered (`hooks/useFxCurrencies.ts`, `app_settings`). One euro-based rate table a day answers every row and any currency added later (`hooks/useLatestRates.ts`); the charted pair keeps its own 5-year series, high/low/average and today-against-average. ECB rates via frankfurter.app, no key, fetched after the page renders. See [`design/market-data/DESIGN.md`](design/market-data/DESIGN.md).
...
- [x] Realtor Insights page (Insights) — a shortlist of listings, each a wide record filled in as you learn it (community, asking price, assessed value, strata, taxes, beds/baths/area/lot/levels/year, parking, orientation, roof/furnace/tank age, windows, renovations, problems, catchment, commute, transit, noise, neighbourhood, for/against, notes, status, rating), with price per sq ft, down payment, CMHC premium, payment and monthly carrying cost derived on the fly from `domain/houseMetrics.ts` (9 tests) — same assumptions for every house, so a comparison is fair, with one tap into the purchase calculator for a real quote. Long-press two or three and compare them side by side, better cell marked green only where "better" is defined and nobody marked on a tie. Community benchmark prices are logged by hand like a tracking account's value, because real-estate boards publish them as documents rather than an API. Migration 033; see [`design/market-data/DESIGN.md`](design/market-data/DESIGN.md).
- [x] An amber "still being built" banner (`components/ui/ExperimentalBanner.tsx`) at the top of the pages whose numbers are not finished being argued with: Loan, Investment, Mortgage, Tax, Exchange, Cost of Living, Realtor and AI Insights. Says it in the app rather than in a release note, so nobody takes a draft figure for a settled one. House Hunt is Realtor Insights with it.

- [x] The standing explanation came off the pages that had stopped needing one: Loan, Investment, Mortgage and Flagged open on the numbers now, with Flagged keeping only the footer note under the list. A paragraph that a returning user scrolls past every time is not orientation, it is a header — and on Flagged it pushed the rows down on every visit. Baby Steps keeps its prose but at the bottom, where the plan now reads as a summary between "Why this order" and "How this page knows"; above the first bar sits a single link out to Ramsey's own account of the seven steps, for the reader who wants the shape of the plan before the numbers.