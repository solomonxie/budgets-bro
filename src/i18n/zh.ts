...
export const zh: Record<keyof typeof en, string> = {
  ...
  'closedAccounts.absorbMessage': '该账户有 {amount} 的已分类支出。并入「{into}」会保留这些记录，并抵消两个账户之间的还款，所有余额和未分配现金都保持不变。直接删除则会让未分配现金减少同样的金额。',
...
'选择',
  ...
  'history.snapshotsHint': '在任何会改写既有数据的操作之前，对整个数据库做的完整备份。当你不确定哪里出错、或数据结构本身变了时用它。',
  ...
  'history.restoreSnapshotMessage': '整个数据库会回到当时的状态，之后的改动全部丢失。恢复后请立即关闭并重新打开应用。',
  'history.restoredTitle': '已恢复',
  'history.restoredMessage': '请关闭并重新打开应用以加载。',
  'history.changesHeading': '变更',
  'history.changesHint': '由数据库自己记录的每一次行改动，包括应用升级所做的改动——那恰恰是最需要能撤销的一类。撤销本身也会被记录，所以可以再撤销回去。',
  ...
  'history.rewindConfirmMessage': '从这里开始的所有改动都会按从新到旧的顺序还原——导入或修复这类会产生大量记录的操作用它。此操作同样会被记录，可以再撤销。',
  'history.undo': '撤销',
  'history.undoConfirmTitle': '撤销 {table} 中的 {count} 行？',
  'history.undoConfirmMessage': '这些行会恢复成原来的样子。此操作同样会被记录，可以再撤销。',
  'nav.history': '历史记录',
};
